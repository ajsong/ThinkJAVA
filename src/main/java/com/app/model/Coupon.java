...
public class Coupon extends Core {
	private final Goods goodsMod;

	public Coupon() {
		this.goodsMod = Common.model(Goods.class);
	}
	...
	public int send(int coupon_id, int member_id) {
		long now = this.now;
		DataMap coupon = Db.name("coupon").where("id='"+coupon_id+"' AND status=1 AND begin_time<='"+now+"' AND (end_time>='"+now+"' OR end_time=0 OR handy_time>0)").find();
		if (coupon != null) {
			if (coupon.getInt("num_per_person")==0 || coupon.getInt("num_per_person")>this.got(coupon_id, member_id)) { //每人限领
				...
				if (coupon.getInt("auto_add")==1) { //自动增加
					...
					success = Db.name("coupon_sn").insert("member_id, coupon_id, coupon_money, sn, add_time, get_time, times, status", member_id, coupon.getInt("id"), coupon.getFloat("coupon_money"), sn, now, now, coupon.getInt("times"), status);
				} else { //由已发行的没领取的优惠券内更新
					success = Db.name("coupon_sn").where("status=1 AND get_time=0 AND member_id=0 AND coupon_id="+coupon_id).order("id ASC").pagesize(1)
	...
	public int got(int coupon_id, int member_id) {
		return Db.name("coupon_sn").where("coupon_id='"+coupon_id+"' AND member_id='"+member_id+"' AND member_id>0").count();
	}
	...
	public void using(String sn, int member_id) {
		...
		DataMap row = Db.name("coupon_sn").where("sn='"+sn+"' AND member_id='"+member_id+"' AND member_id>0").find();
		...
		long now = this.now;
		Db.name("coupon_sn").where("sn='"+sn+"' AND member_id='"+member_id+"' AND member_id>0").update("times, use_time, status", times, now, status);
		Db.name("coupon_history").insert("coupon_sn_id, add_time", row.getInt("id"), now);
	}

	//释放使用优惠券
	public void free(String sn, int member_id) {
		int id = Db.name("coupon_sn").where("sn='"+sn+"' AND member_id='"+member_id+"' AND member_id>0").value("id", Integer.class);
		if (id > 0) {
			Db.name("coupon_sn").where("sn='"+sn+"' AND member_id='"+member_id+"' AND member_id>0").update("status", 0);
			Db.name("coupon_history").delete("coupon_sn_id='"+id+"'");
		}
	}
	...
	public DataList coupons(int member_id, float price, int[] goods) {
		...
		DataList all_coupons = Db.name("coupon_sn cs").innerJoin("coupon c", "cs.coupon_id=c.id").order("cs.id DESC")
	...
	public boolean get_coupon_history(int coupon_sn_id, int day_times) {
		...
		int count = Db.name("coupon_history").where("coupon_sn_id='"+coupon_sn_id+"' AND add_time>='"+now1+"' AND add_time<='"+now2+"'").count();
	...
	public int[] get_coupon_goods(int coupon_id) {
		...
		DataList goods = Db.name("coupon_goods").where("coupon_id='"+coupon_id+"'").select("goods_id");
		if (goods != null) {
			for (DataMap g : goods) {
		...
		if (buy_goods != null && buy_goods.length > 0) {
			...
			if (Integer.parseInt(this.configs.get("G_COUPON_PERMIT_CATEGORY"))==1) {
				...
				for (int goods_id : buy_goods) {
					int category_id = Db.name("goods").where(goods_id).value("category_id", Integer.class);
	...
	public int[] get_goods(int[] coupon_goods) {
		if (Integer.parseInt(this.configs.get("G_COUPON_PERMIT_CATEGORY"))==1) {
			...
			for (String category_id : categories) {
				DataList rs = Db.name("goods").where("category_id='"+category_id+"'").select("id");
				if (rs != null) {
					for (DataMap g : rs) {
