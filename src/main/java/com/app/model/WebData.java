...
public class WebData extends Core {
	static String name = "webdata";

	//数据库操作(自动设定表名)===================================================
	public static String connectname() {
		String connection = "";
		Field[] fields = WebData.class.getDeclaredFields();
		try {
			for (Field field : fields) {
				field.setAccessible(true);
				if (Modifier.isStatic(field.getModifiers())) {
					if (field.getName().equals("connection")) connection = (String) field.get(WebData.class);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static String tablename() {
		...
		try {
			for (Field field : fields) {
				field.setAccessible(true);
				if (Modifier.isStatic(field.getModifiers())) {
					if (field.getName().equals("name")) name = (String) field.get(WebData.class);
					if (field.getName().equals("table")) table = (String) field.get(WebData.class);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
