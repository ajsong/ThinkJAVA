...
public class Goods extends Core {
	private final Coupon couponMod;

	public Goods() {
		this.couponMod = Common.model(Coupon.class);
	}
	...
	public DataMap detail(int goods_id, boolean show_origin_pic, String status) {
		...
		DataMap row = Db.name("goods g").where("g.id='"+goods_id+"' "+where)
			.find("g.*, NULL as pics, NULL as specs, '' as spec, NULL as country, '' as sale_method_name, 0 as favorited," +
		...
		if (row != null) {
			...
			DataMap memberObj = sessionDataMap("member");
			row.put("favorited", memberObj != null ? this.favorited(goods_id, memberObj.getInt("id")) : 0);
	...
	public float get_groupbuy_price(int goods_id, String spec, int member_id) {
		long now = this.now;
		DataMap row = Db.name("goods").where(goods_id).find("groupbuy_price, groupbuy_begin_time, groupbuy_end_time, groupbuy_amount, groupbuy_count, groupbuy_limit");
		if (row == null) return 0;
		if (spec.length() > 0) {
			DataMap _spec = Db.name("goods_spec").where("goods_id='"+goods_id+"' AND spec='"+spec+"'").find("groupbuy_price");
			if (_spec != null) {
		...
		if (member_id<=0) {
			DataMap memberObj = sessionDataMap("member");
			if (memberObj != null) member_id = memberObj.getInt("id");
		}
		if (row.getInt("groupbuy_limit")>0 && member_id>0) {
			int count = Db.name("order_goods og").leftJoin("order o", "og.order_id=o.id").where("goods_id='"+goods_id+"' AND og.member_id='"+member_id+"' AND o.type=1").count();
	...
	public float get_purchase_price(int goods_id, String spec, int member_id) {
		long now = this.now;
		DataMap row = Db.name("goods").where(goods_id).find("purchase_price, purchase_begin_time, purchase_end_time, purchase_amount, purchase_count, purchase_limit");
		if (row == null) return 0;
		if (spec.length() > 0) {
			DataMap _spec = Db.name("goods_spec").where("goods_id='"+goods_id+"' AND spec='"+spec+"'").find("purchase_price");
			if (_spec != null) {
		...
		if (row.getInt("purchase_limit")>0 && member_id>0) {
			int count = Db.name("order_goods og").leftJoin("order o", "og.order_id=o.id").where("goods_id='"+goods_id+"' AND og.member_id='"+member_id+"' AND o.type=2").count();
	...
	public float get_chop_price(int goods_id, String spec, int member_id) {
		long now = this.now;
		DataMap row = Db.name("goods").where(goods_id).find("chop_price, chop_begin_time, chop_end_time, chop_amount, chop_count");
		if (row == null) return 0;
		if (spec.length() > 0) {
			DataMap _spec = Db.name("goods_spec").where("goods_id='"+goods_id+"' AND spec='"+spec+"'").find("chop_price");
			if (_spec != null) {
	...
	public DataMap get_country(int country_id) {
		if (country_id > 0) {
			return Db.name("country").where(country_id).find("id, name, flag_pic");
		}
		return null;
	}
	...
	public DataList get_pics(int goods_id, boolean show_origin_pic) {
		DataList pics = Db.name("goods_pic").where("goods_id='"+goods_id+"'").order("id ASC").select();
	...
	public DataList get_specs(int goods_id, boolean allways_show) {
		if (!allways_show) {
			String spec = Db.name("goods_spec").where("goods_id='"+goods_id+"'").value("spec");
			if (spec.length() == 0 || spec.equals("默认规格")) return null;
		}
		DataList rs = Db.name("goods_spec_linkage l").leftJoin("goods_spec_category c", "l.spec_id=c.id")
			.where("l.goods_id='"+goods_id+"' AND l.parent_id='0'").order("l.id ASC").select("l.spec_id as id, c.name, NULL as sub");
		if (rs != null) {
			for (int i = 0; i < rs.size(); i++) {
				DataMap g = rs.get(i);
				rs.set(i, "sub", Db.name("goods_spec_linkage l").leftJoin("goods_spec_category c", "l.spec_id=c.id")
					.where("l.goods_id='"+goods_id+"' AND l.parent_id='"+g.getInt("id")+"'").order("l.id ASC").select("l.spec_id as id, c.name"));
			}
		}
		return rs;
	}
	...
	public DataList get_coupons(int goods_id, int shop_id) {
		...
		DataList rs = Db.name("coupon")
			...
			.order("id ASC").select("*, '' as time_memo, '' as min_price_memo");
		DataList _private = Db.name("coupon c").innerJoin("coupon_goods cg", "c.id=cg.coupon_id")
			.where("shop_id='"+shop_id+"' AND type=0 AND permit_goods=1 AND goods_id='"+goods_id+"' AND c.status=1 AND begin_time<='"+now+"' AND (end_time>='"+now+"' OR end_time=0 OR handy_time>0)").order("c.id ASC").select("c.*, '' as time_memo, '' as min_price_memo");
	...
	public float get_spec_price(int goods_id, String spec) {
		DataMap row = Db.name("goods_spec").where("goods_id='"+goods_id+"' AND spec='"+spec+"'").find("price, promote_price");
	...
	public int favorited(int goods_id, int member_id) {
		int favorited = 0;
		if (member_id > 0) {
			favorited = Db.name("favorite").where("item_id='"+goods_id+"' AND member_id='"+member_id+"'").count();
		}
		return favorited;
	}
	...
	public DataList get_categories(int parent_id) {
		DataList rs = Db.name("goods_category").where("status=1 AND parent_id='{$parent_id}'").order("sort ASC, id ASC").select("*, NULL as categories");
		if (rs != null) {
			for (int i = 0; i < rs.size(); i++) {
	...
	public String get_category_parents_tree(int category_id) {
		...
		DataMap row = Db.name("goods_category").where("status=1 AND id='"+category_id+"'").find("parent_id");
	...
	public String get_category_children_tree(int category_id) {
		...
		DataList rs = Db.name("goods_category").where("status=1 AND parent_id='"+category_id+"'").select("id");
		if (rs != null) {
			for (DataMap g : rs) {
				ids.append(",").append(g.getInt("id"));
				int count = Db.name("goods_category").where("status=1 AND parent_id='"+g.getInt("id")+"'").count();
