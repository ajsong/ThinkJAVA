...
public class ArticleCategory extends Core {
	...
	public static String connectname() {
		String connection = "";
		Field[] fields = ArticleCategory.class.getDeclaredFields();
		try {
			for (Field field : fields) {
				field.setAccessible(true);
				if (Modifier.isStatic(field.getModifiers())) {
					if (field.getName().equals("connection")) connection = (String) field.get(ArticleCategory.class);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static String tablename() {
		...
		try {
			for (Field field : fields) {
				field.setAccessible(true);
				if (Modifier.isStatic(field.getModifiers())) {
					if (field.getName().equals("name")) name = (String) field.get(ArticleCategory.class);
					if (field.getName().equals("table")) table = (String) field.get(ArticleCategory.class);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
