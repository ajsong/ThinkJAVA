...
public class Manage extends Core {
	...
	public static String connectname() {
		String connection = "";
		Field[] fields = Manage.class.getDeclaredFields();
		try {
			for (Field field : fields) {
				field.setAccessible(true);
				if (Modifier.isStatic(field.getModifiers())) {
					if (field.getName().equals("connection")) connection = (String) field.get(Manage.class);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static String tablename() {
		...
		try {
			for (Field field : fields) {
				field.setAccessible(true);
				if (Modifier.isStatic(field.getModifiers())) {
					if (field.getName().equals("name")) name = (String) field.get(Manage.class);
					if (field.getName().equals("table")) table = (String) field.get(Manage.class);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
