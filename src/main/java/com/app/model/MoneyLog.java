...
public class MoneyLog extends Core {
	...
	public static String connectname() {
		String connection = "";
		Field[] fields = MoneyLog.class.getDeclaredFields();
		try {
			for (Field field : fields) {
				field.setAccessible(true);
				if (Modifier.isStatic(field.getModifiers())) {
					if (field.getName().equals("connection")) connection = (String) field.get(MoneyLog.class);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static String tablename() {
		...
		try {
			for (Field field : fields) {
				field.setAccessible(true);
				if (Modifier.isStatic(field.getModifiers())) {
					if (field.getName().equals("name")) name = (String) field.get(MoneyLog.class);
					if (field.getName().equals("table")) table = (String) field.get(MoneyLog.class);
				}
			}
		} catch (Exception e) {
			e.printStackTrace();
