...
public class DataList implements Iterable<DataMap> {
	...
	public String toString() {
		return JSON.toJSONString(DataMap.dataToMap(this.list));
	}
	public DataList each(CallbackEach callback) {
		for (int i = 0; i < this.list.size(); i++) {
			if (callback instanceof CallbackDataMap) ((CallbackDataMap)callback).get(this.list.get(i));
			else if (callback instanceof CallbackDataMapIndex) ((CallbackDataMapIndex)callback).get(this.list.get(i), i);
		}
		return this;
	}
	@Override
