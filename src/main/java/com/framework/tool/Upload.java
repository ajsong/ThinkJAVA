//Developed by @mario 1.4.20220706
...
public class Upload {
	...
	public Map<String, Object> file(String dir, String fileType, Map<String, Object> thirdParty, boolean returnDetail) {
		...
		String uploadDir = Common.getYml("spring.servlet.upload.path", "uploads") + (dir.length() > 0 ? "/" + Common.trim(dir, "/") : "");
		int maxMemSize = Common.getYml("spring.servlet.upload.memorysize", 10000) * 1024;
		long maxFileSize = Common.getYml("spring.servlet.upload.filesize", Long.parseLong("10000")) * 1024;
		//获取文件上传目录路径，在项目部署路径下的uploads目录里，若想让浏览器不能直接访问到图片，可以放在WEB-INF下
		String rootPath = Common.root_path();
			...
			try {
				...
				for (FileItem item: fileItems) {
					...
					if (returnDetail) {
						...
						if (fileTypes.length > 0 && Arrays.asList(fileTypes).contains(suffix)) {
							int width, height;
							InputStream stream = Files.newInputStream(new File(filePath, filename).toPath());
